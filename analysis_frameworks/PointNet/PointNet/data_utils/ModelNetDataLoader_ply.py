...
def write_ply_ascii_geo(filedir, coords):
    ...
    coords = coords
    for p in coords:
        f.writelines(f'{int(p[0])} {int(p[1])} {int(p[2])}\n')
    f.close() 
...
class ModelNetDataLoader_ply(Dataset):
    def __init__(self, root, split_path, args, split='train', process_data=False):
        ...
        self.datapath = [(shape_names[i], os.path.join(self.root, shape_names[i], split, shape_ids[split][i]) + '.ply') for i
        ...
        if self.process_data:
            if not os.path.exists(self.save_path):
                ...
                for index in tqdm(range(len(self.datapath)), total=len(self.datapath)):
                    ...
                    point_set = read_ply_ascii_geo(fn[1]).astype(np.float32)
...
if __name__ == '__main__':
    ...
    data = ModelNetDataLoader_ply('/data/modelnet40_ply_downsampled/', split='train')
