...
class _VoxceptionResNet(nn.Module):
  ...
  def __init__(self, num_filters, in_chs, activation=F.relu):
    ...
    #path_1
    self.conv1_1 = nn.Conv3d(in_channels=in_chs, out_channels=int(num_filters/4),kernel_size=3, stride=1, padding=1, bias=True)
    self.conv1_2 = nn.Conv3d(in_channels=int(num_filters/4), out_channels=int(num_filters/2),kernel_size=3, stride=1, padding=1, bias=True)
    #path_2
    self.conv2_1 = nn.Conv3d(in_channels=in_chs, out_channels=int(num_filters/4),kernel_size=1, stride=1, padding=0, bias=True)
    self.conv2_2 = nn.Conv3d(in_channels=int(num_filters/4), out_channels=int(num_filters/4),kernel_size=3, stride=1, padding=1, bias=True)
    self.conv2_3 = nn.Conv3d(in_channels=int(num_filters/4), out_channels=int(num_filters/2),kernel_size=1, stride=1, padding=0, bias=True)
  ...
  def forward(self, x):
    ...
    # concat paths
    residual = torch.cat((tensor1_2,tensor2_3),dim=1) #pytorch的格式，dim应为1，residual.shape: (1, 32, 32, 32, 32)
    # add & relu
...
class AnalysisTransform(nn.Module):
  ...
  def __init__(self):
    ...
    self.in_chs = 1
    self.conv_in = nn.Conv3d(in_channels=self.in_chs, out_channels=16,kernel_size=3, stride=1, padding=1, bias=True)
    ...
    self.down_1 = nn.Conv3d(in_channels=16, out_channels=32,kernel_size=3, stride=2, padding=0, bias=False)
    ...
    self.down_2 = nn.Conv3d(in_channels=32, out_channels=64,kernel_size=3, stride=2, padding=0, bias=False)
    ...
    self.conv_out = nn.Conv3d(in_channels=64, out_channels=16,kernel_size=3, stride=1, padding=1, bias=True)
  ...
  def forward(self, x):
    ...
    feature1_3 = torch.nn.functional.pad(feature1_3, pad=(0,1,0,1,0,1), mode='constant', value=0) #padding:top 0 bottom 1 left 0 right 1
    ...
    feature2_3 = torch.nn.functional.pad(feature2_3, pad=(0,1,0,1,0,1), mode='constant', value=0) #padding:top 0 bottom 1 left 0 right 1
...
class SynthesisTransform(nn.Module):
  def __init__(self):
    ...
    def vrn_block(num_filters, vrnin_chs):
      return _VoxceptionResNet(num_filters, vrnin_chs)
    self.in_chs = 16    
    self.deconv_in = nn.Conv3d(in_channels=self.in_chs, out_channels=64,kernel_size=3, stride=1, padding=1, bias=True)
    ...
    self.up_1 = nn.ConvTranspose3d(in_channels=64, out_channels=32, kernel_size=3, stride=2, padding=1,output_padding=1, bias=True)
    self.vrn2_1 = vrn_block(num_filters=32, vrnin_chs=32)
    self.vrn2_2 = vrn_block(num_filters=32, vrnin_chs=32)
    self.vrn2_3 = vrn_block(num_filters=32, vrnin_chs=32)

    self.up_2 = nn.ConvTranspose3d(in_channels=32, out_channels=16, kernel_size=3, stride=2, padding=1,output_padding=1, bias=True)
    ...
    self.deconv_out = nn.Conv3d(in_channels=16, out_channels=1,kernel_size=3, stride=1, padding=1, bias=True)
    '''针对tf的测试结果(batch, depth, height, width, channels)
...
class HyperEncoder(nn.Module):
  ...
  def __init__(self, activation=F.relu):
    ...
    self.activation = activation
    self.in_chs = 16    
    self.conv1 = nn.Conv3d(in_channels=self.in_chs, out_channels=16,kernel_size=3, stride=1, padding=1, bias=True)
    self.conv2 = nn.Conv3d(in_channels=16, out_channels=16,kernel_size=3, stride=2, padding=0, bias=True)
    self.conv3 = nn.Conv3d(in_channels=16, out_channels=8,kernel_size=3, stride=1, padding=1, bias=True)
    '''针对tf的测试结果(batch, depth, height, width, channels)
    x.shape: (1, 16, 16, 16, 16)
  ...
  def forward(self, x):
    ...
    f1 = torch.nn.functional.pad(f1, pad=(0,1,0,1,0,1), mode='constant', value=0)
    f2 = self.conv2(f1)
...
class HyperDecoder(nn.Module):
  ...
  def __init__(self, activation=F.relu):
    ...
    self.in_chs = 8    
    self.conv1 = nn.Conv3d(in_channels=self.in_chs, out_channels=16,kernel_size=3, stride=1, padding=1, bias=True)
    self.conv2 = nn.ConvTranspose3d(in_channels=16, out_channels=16, kernel_size=3, stride=2, padding=1,output_padding=1, bias=True)
    self.conv3 = nn.Conv3d(in_channels=16, out_channels=32,kernel_size=3, stride=1, padding=1, bias=True)
    self.conv4_1 = nn.Conv3d(in_channels=32, out_channels=16,kernel_size=3, stride=1, padding=1, bias=True)
    self.conv4_2 = nn.Conv3d(in_channels=32, out_channels=16,kernel_size=3, stride=1, padding=1, bias=True)
    '''针对tf的测试结果(batch, depth, height, width, channels)
    x.shape: (1, 8, 8, 8, 8)
    f1.shape: (1, 8, 8, 8, 16)
...
if __name__=='__main__':
    ...
    print("hyper_prior.shape:",hyper_prior.shape) #torch.Size([1, 8, 8, 8, 8])
    loc, scale = hyper_decoder(hyper_prior)
    print("loc.shape:",loc.shape, ", scale.shape:",scale.shape) #loc.shape: torch.Size([1, 16, 16, 16, 16]) , scale.shape: torch.Size([1, 16, 16, 16, 16])
