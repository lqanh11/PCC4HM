...
from gpcc import gpcc_encode
from data_utils import array2vector, write_ply_ascii_geo
    ...
    def test(self, dataloader, main_tag='Test'):
        with torch.no_grad():
            ...
            for idx, batch in enumerate(tqdm(dataloader)):
                ...
                z_F = z.F[indices_sort]

                coords = (z_C//8).detach().cpu()[:,1:]
                coords = coords.numpy().astype('int')

                write_ply_ascii_geo(filedir='z_coords.ply', coords=coords)
                gpcc_encode('z_coords.ply', 'z_coords.bin')

                ### features
                base_strings, base_min_v, base_max_v = self.model.entropy_bottleneck_b.compress(z_F)
                base_shape = z_F.shape
                ...
                ## decoder part
                logits = self.model.classifier(z_q)

                bits_b = np.array([
                                    os.path.getsize('z_coords.bin')*8,
