...
################################################################################
###二次转换，找到之前读取错误的文件，更改第一行后重新打开，再转换
################################################################################
import logging
...
def process(path, args):
    ...
    if not os.path.exists(target_path):
        ...
        with open(ori_path, 'r') as file:
            ...
            first_line = lines[0].replace('OFF','OFF\r\n')
            print(first_line)
            lines = [first_line] + lines[1:]
        with open(ori_path, 'w+') as file:#'/userhome/pcc_geo_cnn_v1/ModelNet40_pc_64/sofa/test/test.off'
            for line in lines:
                file.write(line)
        try:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source', type=str,default='/media/avitech/Data/quocanhle/PointCloud/dataset/modelnet40/mesh_original_format_off/', help='Source directory')
    parser.add_argument('--dest', type=str,default='/media/avitech/Data/quocanhle/PointCloud/Point_Cloud_Compression/pcc_geo_cnn/pcc_geo_cnn_v1_yh/pytorch/ModelNet40_pc_64/', help='Destination directory')
