...
def write_binary_files_hyper(filename, y_strings, z_strings, points_numbers, cube_positions, y_min_vs, y_max_vs, y_shape, z_min_v, z_max_v, z_shape, rootdir='./'):
  ...
  with open(file_strings_head, 'wb') as f:
    ...
    f.write(np.array(y_shape, dtype=np.int16).tobytes())# [batch size, channels, length, width, height]
...
def read_binary_files_hyper(filename, rootdir='./'):
  ...
  with open(file_strings_head, 'rb') as f:
    ...
    y_max_min_vs = np.frombuffer(f.read(y_strings_num*2), dtype=np.uint16).astype('int32') #uint16所以要*2
  ...
  return y_strings, z_strings, points_numbers, cube_positions, y_min_vs[0], y_max_vs[0], y_shape, z_min_v, z_max_v, z_shape
