...
def load_points(filename, cube_size=64, min_num=20):
  ...
  set_points = []
  # orderd
  step = cube_positions.max() + 1 #所有维度中最大的1个数
  cube_positions_n = cube_positions[:,0:1] + cube_positions[:,1:2]*step + cube_positions[:,2:3]*step*step
  cube_positions_n = np.sort(cube_positions_n, axis=0) #从小到大
...
def voxels2points(voxels):
  ...
  voxels = np.squeeze(np.uint8(voxels)) # 0 or 1 从数组的形状中删除单维度条目，即把shape中为1的维度去掉
  set_points = []
  for _, vol in enumerate(voxels): #vol.shape: (64, 64, 64)
    points = np.array(np.where(vol>0)).transpose((1,0)) #points.shape: (1266, 3)
...
def get_adaptive_thres(vol, num, init_thres=-2.0):
  ...
  if values.shape[0] < num: #这里逻辑有问题，因为values是数组，无论如何都要reshape
