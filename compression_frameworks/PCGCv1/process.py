...
def preprocess(input_file, scale, cube_size, min_num): #min_num=64
    ...
    """
    prefix=input_file.split('/')[-1].split('_')[0]+str(random.randint(1,100)) #redandblack2
    print('===== Preprocess =====')
    # scaling (optional)
    start = time.time()
    if scale == 1:
        ...
        write_ply_data(scaling_file, pc_down) #写入xyz到ply
    ...
    points_numbers = np.sum(cubes, axis=(1,2,3,4)).astype(np.uint16) #每个cube里有多少个点
    print("Voxelization: {}s".format(round(time.time()-start, 4)))

    print('cubes shape: {}'.format(cubes.shape)) #(189, 64, 64, 64, 1)
    print('points numbers (sum/mean/max/min): {} {} {} {}'.format( 
    points_numbers.sum(), round(points_numbers.mean()), points_numbers.max(), points_numbers.min())) #757478 4008.0 13906 68
...
def postprocess(output_file, cubes, points_numbers, cube_positions, scale, cube_size, rho, fixed_thres=None):
    ...
    # Classify.
    start = time.time()
    output = select_voxels(cubes, points_numbers, rho, fixed_thres=fixed_thres) #output是针对每个cube的mask，1为occupied
