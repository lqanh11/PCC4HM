...
def compress(input_filename,outputpath):
    start = time.time()
    file_name = os.path.split(input_filename)[1] #文件名
    outputpath += os.path.splitext(file_name)[0] + '/'
    os.makedirs(outputpath,exist_ok=True)
    dec_filename = outputpath + file_name.replace('.ply','_dec.ply')
    input_normal_file = [input_filename]
    output_filename = outputpath + file_name.replace('.ply','.bin')
    input_filename = [input_filename]
    if 'vox10' in file_name:
        args_resolution = 1024
        octree_level = 4
    elif 'vox9' in file_name:
        args_resolution = 512
        octree_level = 3
    elif 'vox11' in file_name:
    ...
    with_normals = input_normal_file is not None #true
    ...
    #     assert files_mult * len(input_filename) == len(output_filename) #output_files编码输出文件
    #     assert files_mult * len(input_normal_file) == len(output_filename)
    # else:
    #     assert files_mult * len(input_filename) == len(output_filename)
    decode_files = dec_filename is not None
    # if decode_files:
    #     assert files_mult * len(input_filename) == len(dec_filename)
    ...
    points = pc_io.load_points(input_filename, batch_size=args.read_batch_size) #points.shape: (1, 757691, 3)只有xyz
    if with_normals:
        normals = [PyntCloud.from_file(x).points[['nx', 'ny', 'nz']].values for x in input_normal_file]
    ...
    dense_tensor_shape[1:] = dense_tensor_shape[1:] // (2 ** octree_level)
    blocks_list, binstr_list = zip(*[partition_octree(p, bbox_min, bbox_max, octree_level) for p in points])
    ...
    print(f'Processing resolution {args_resolution} with octree level {octree_level} resulting in '
    ...
    ## Encoder
    for i in trange(len(input_filename)): #trange进度条
        ori_file, cur_points, blocks, binstr = [x[i] for x in (input_filename, points, blocks_list, binstr_list)] #blocks_list：[1,202,1845,6],这不是数组，1845不是固定的
        n_blocks = len(blocks) #202

        cur_output_files = [output_filename] #2个
        if decode_files:
            cur_dec_files = [dec_filename] #2个
        ...
        data_list, data, _ = compress_blocks(model,x_shape, blocks, binstr, cur_points, args_resolution,
                                                                octree_level, with_normals=with_normals,
        ...
        for j in range(len(cur_output_files)): #2个
            ...
            with gzip.open(of, "wb") as f:
                ret = save_compressed_file(binstr, cur_data_list, args_resolution, octree_level)
                f.write(ret)
            if decode_files:
    ...
    pos_total_size_in_bytes = os.stat(output_filename).st_size
    result = {}
    result['Encodedfile'] = file_name
    result['Enc_dec_time'] = end - start
    result['bpp'] = pos_total_size_in_bytes * 8 / points[0].shape[0]
    result['d1'] = data[0]['metrics']['d1_psnr']
    result['d2'] = data[0]['metrics']['d2_psnr']
    return result
...
if __name__ == '__main__':
    ...
    # dec_filename = [dec_filename]
    outputpath = '/media/avitech/CODE/quocanhle/Point_Cloud_Compression/compression_frameworks/GEO_CNN/pcc_geo_cnn_v2_yehua/pytorch/results/'
    os.makedirs(outputpath,exist_ok=True)
    csv_file = outputpath + 'result.csv'
    csv_result = []
    paths = [
        '/media/avitech/CODE/quocanhle/Point_Cloud_Compression/dataset/testdata/MVUB/sarah_vox9_frame0023.ply'
#         '/userhome/dataset/paper_test/added/sarah_vox10_0023.ply',
        # '/userhome/dataset/paper_test/added/sarah_vox9_0023.ply',
        # '/userhome/dataset/paper_test/added/phil_vox9_0139.ply','/userhome/dataset/paper_test/added/phil_vox10_0139.ply',
        # '/userhome/dataset/paper_test/redandblack_vox10_1550.ply', '/userhome/dataset/paper_test/queen_vox10_0200.ply',
        # '/userhome/dataset/paper_test/longdress_vox10_1300.ply', '/userhome/dataset/paper_test/basketball_player_vox11_00000200.ply',
        # '/userhome/dataset/paper_test/loot_vox10_1200.ply',
        # '/userhome/dataset/paper_test/dancer_vox11_00000001.ply', '/userhome/dataset/paper_test/soldier_vox10_0690.ply'
        ]
    for i,input_file in enumerate(paths):
        print(input_file)
        results = compress(input_file,outputpath)
