    ...
    def test(self, dataloader, main_tag='Test'):
        with torch.no_grad():
            ...
            for idx, batch in enumerate(tqdm(dataloader)):
                ...
                nums_latentspace = [[len(C) for C in latentspace.decomposed_coordinates] for latentspace in [y_q]]

                # # Forward.
                ## encoder part
                out_cls_backbone = self.model.classifier_backbone(y_q)
                out_cls_backbone_q, likelihood_b = self.model.get_likelihood_b(out_cls_backbone[:,:,0],
                         quantize_mode="symbols")
                
                base_strings, base_min_v, base_max_v = self.model.entropy_bottleneck_b.compress(out_cls_backbone[:,:,0])
                base_shape = torch.squeeze(out_cls_backbone).shape
                ...
                logits = self.model.classifier_mlp(torch.unsqueeze(out_cls_backbone_q,-1))
