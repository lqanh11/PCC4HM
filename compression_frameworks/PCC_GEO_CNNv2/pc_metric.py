...
@njit
def assign_attr(attr1, idx1, idx2): #双向遍历，求均值
    ...
    """
    counts = np.zeros(idx1.shape[0]) #表示x2有多少个点
...
def compute_metrics(p1, p2, r, p1_n=None, t1=None):
    ...
    _, idx2 = t2.query(p1) ## 返回最近邻点的距离d和在数组中的索引x
    _, idx1 = t1.query(p2)

    max_energy = 3 * r * r
    p1_ngb = p2[idx2] #在解码点中与原始点最接近的点
    p2_ngb = p1[idx1] #在原始点中与解码点最接近的点
    d1_sum_AB = sum_d1(p1, p1_ngb) #原始点与解码点差的平方和
    d1_sum_BA = sum_d1(p2, p2_ngb) #解码点与原始点差的平方和
    ...
    d1_mse = max(d1_mse_AB, d1_mse_BA) #越大说明误差越大，选误差大的才能说明问题
    d1_psnr_AB = psnr(d1_mse_AB, max_energy)
    d1_psnr_BA = psnr(d1_mse_BA, max_energy)
    d1_psnr = min(d1_psnr_AB, d1_psnr_BA) #越小说明误差越大
    metrics = {
        'd1_sum_AB': d1_sum_AB,
        'd1_sum_BA': d1_sum_BA,
    ...
    if p1_n is not None:
        ...
        p2_n = assign_attr(p1_n, idx1, idx2) #双向遍历，求均值
        p1_ngb_n = p2_n[idx2]
        p2_ngb_n = p1_n[idx1]
